...
<header class="fixed top-0 w-full z-50 bg-[#090909]/60 backdrop-blur-xl shadow-[0_20px_40px_rgba(0,0,0,0.4)]" x-data="{ mobileMenuOpen: false }">
  <nav class="flex justify-between items-center px-8 h-20 w-full font-['Nunito'] tracking-tighter">
    <div class="flex items-center gap-8">
      <a class="flex items-center gap-3 decoration-transparent" href="{{ route("homepage.html") }}" aria-label="FS86 Sahiwal Home">
          <div class="flex items-center justify-center w-[72px] h-12 rounded-xl bg-[#141414]/90 border border-white/10 overflow-hidden shadow-[0_4px_10px_rgba(0,0,0,0.5)] hover:shadow-[0_6px_15px_rgba(249,115,22,0.3)] hover:scale-105 transition-all duration-300">
            <img src="{{ asset('assets/image/logo.png') }}" alt="FS Logo" class="w-full h-full object-cover">
          </div>
        </a>
      <div class="hidden md:flex items-center gap-4 text-sm">
        <a class="text-white/70 hover:text-primary transition-colors px-3 py-1 rounded hover:bg-white/10" href="{{ route("homepage.html") }}">Home</a>
        <a class="text-white/70 hover:text-primary transition-colors px-3 py-1 rounded hover:bg-white/10" href="{{ route("shop_page.html") }}">Shop</a>
        <a class="text-white/70 hover:text-primary transition-colors px-3 py-1 rounded hover:bg-white/10" href="{{ route("about_us.html") }}">About</a>
        <a class="text-white/70 hover:text-primary transition-colors px-3 py-1 rounded hover:bg-white/10" href="{{ route("store_locator.html") }}">Stores</a>


        <a class="text-white/70 hover:text-primary transition-colors px-3 py-1 rounded hover:bg-white/10" href="{{ route('faqs.html') }}">FAQs</a>
      </div>
    </div>
    <div class="flex items-center gap-4">
      <a href="{{ route("wishlist.html") }}" class="p-2 hover:bg-white/10 transition-all duration-300 scale-95 active:scale-90" title="Wishlist">
        <span class="material-symbols-outlined text-[#f97316]">favorite</span>
      </a>

      {{-- ── Global Product Search (Alpine.js) ─────────────────── --}}
      <div x-data="{
              query: '',
              results: [],
              isOpen: false,
      ...
      {{-- ── End Global Product Search ────────────────────────── --}}

      <a href="{{ route("cart_checkout.html") }}" class="p-2 hover:bg-white/10 transition-all duration-300 scale-95 active:scale-90 relative" title="Shopping Cart">
      ...
      <a href="{{ route("my_account.html") }}" class="p-2 hover:bg-white/10 transition-all duration-300 scale-95 active:scale-90" title="My Account">
        <span class="material-symbols-outlined text-[#f97316]">person</span>
      </a>
      <a id="auth-link" href="{{ route("login_signup.html") }}" class="p-2 hover:bg-white/10 transition-all duration-300 scale-95 active:scale-90" title="Login/Signup">
  ...
  >
    <div class="flex flex-col p-6 gap-4 font-['Nunito'] tracking-tighter">
      <a @click="mobileMenuOpen = false" class="text-white/80 hover:text-[#f97316] hover:bg-white/5 transition-all px-4 py-3 rounded-lg text-lg font-semibold" href="{{ route("homepage.html") }}">Home</a>
      <a @click="mobileMenuOpen = false" class="text-white/80 hover:text-[#f97316] hover:bg-white/5 transition-all px-4 py-3 rounded-lg text-lg font-semibold" href="{{ route("shop_page.html") }}">Shop</a>
      <a @click="mobileMenuOpen = false" class="text-white/80 hover:text-[#f97316] hover:bg-white/5 transition-all px-4 py-3 rounded-lg text-lg font-semibold" href="{{ route("about_us.html") }}">About</a>
      <a @click="mobileMenuOpen = false" class="text-white/80 hover:text-[#f97316] hover:bg-white/5 transition-all px-4 py-3 rounded-lg text-lg font-semibold" href="{{ route("store_locator.html") }}">Stores</a>
      <a @click="mobileMenuOpen = false" class="text-white/80 hover:text-[#f97316] hover:bg-white/5 transition-all px-4 py-3 rounded-lg text-lg font-semibold" href="{{ route('faqs.html') }}">FAQs</a>
    </div>
  </div>
</header>
