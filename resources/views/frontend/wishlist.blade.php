...
<div id="wishlist-empty" class="hidden text-center py-20">
  ...
  <a href="{{ route('shop_page.html') }}" class="px-8 py-3 bg-gradient-to-br from-primary to-error text-white shadow-[0_4px_10px_rgba(249,115,22,0.4)] hover:shadow-[0_6px_15px_rgba(249,115,22,0.6)] font-label text-xs uppercase tracking-widest">Browse Shop</a>
</div>
...
<div id="wishlist-login" class="hidden text-center py-20">
  ...
  <a href="{{ route('login_signup.html') }}" class="px-8 py-3 bg-gradient-to-br from-primary to-error text-white shadow-[0_4px_10px_rgba(249,115,22,0.4)] hover:shadow-[0_6px_15px_rgba(249,115,22,0.6)] font-label text-xs uppercase tracking-widest">Login / Sign Up</a>
</div>
</main>
...
<script>
  async function loadWishlist() {
    ...
    if (res.success && res.data && res.data.length > 0) {
      ...
      // Discover more card
      html += `
      <article class="flex flex-col items-center justify-center border-2 border-dashed border-outline-variant/20 p-12 text-center bg-surface/50 rounded-xl overflow-hidden">
        ...
        <a href="{{ route('shop_page.html') }}" class="px-6 py-2 border border-outline-variant text-on-surface font-label text-[10px] tracking-widest uppercase hover:bg-white hover:text-black transition-all">Explore Shop</a>
      </article>`;
      grid.innerHTML = html;
    } else {
      grid.classList.add('hidden');
